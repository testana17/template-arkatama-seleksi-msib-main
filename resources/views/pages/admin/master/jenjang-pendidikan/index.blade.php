...
@section('content')
    <div class="card-body py-4">
        <div class="d-flex flex-md-row flex-column gap-4  align-items-center justify-content-between">
            <div class="search-box">
                ...
                <input type="text" data-table-id="jenjang-pendidikan-table" id="searchBox" data-action="search"
            ...
            <div class="d-flex  flex-row justify-content-center justify-content-md-end w-100 gap-1">
                @if (request()->routeIs('master.jenjang-pendidikan.histori'))
                    <a href="{{ route('master.jenjang-pendidikan.index') }}" class="btn btn-primary">
                        <i class="fal fa-arrow-left fs-3"></i>
                        <span class="ms-2">
                            Kembali
                        </span>
                    </a>
                @else
                    @can($globalModule['create'])
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-jenjang-pendidikan_modal"
                    ...
                    @can($globalModule['read'])
                        <a href="{{ route('master.jenjang-pendidikan.histori') }}" class="btn btn-warning">
    ...
    @include('pages.admin.master.jenjang-pendidikan.partials.modals')

    @push('scripts')
        {{ $dataTable->scripts() }}
        <script>
            const tableId = 'jenjang-pendidikan-table';

            function generateSingkatan(namaJenjang) {
                const words = namaJenjang.split(' ');
                let singkatan = '';
                for (let i = 0; i < words.length; i++) {
                    if (words[i].length > 0) {
                        singkatan += words[i][0].toUpperCase();
                    }
                }
                return singkatan;
            }
            ...
            inputNama.addEventListener('input', function(event) {
                ...
                inputKode.value = generateSingkatan(event.target.value);
            });
            ...
            inputNama2.addEventListener('input', function(event) {
                validateInputNameJenjang(event.target);
                inputKode2.value = generateSingkatan(event.target.value);
            });
