...
@section('content')
        ...
        <div class="d-flex flex-md-row flex-column gap-4  align-items-center justify-content-between">
            ...
            <div class="d-flex flex-row justify-content-center justify-content-md-end w-100 gap-1">
                @if (request()->routeIs('master.kecamatan.histori'))
                    <a href="{{ route('master.kecamatan.index') }}" class="btn btn-primary">
                ...
                @else
                    @can($globalModule['create'])
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-kecamatan_modal"
                    ...
                    @can($globalModule['read'])
                        <a href="{{ route('master.kecamatan.histori') }}" class="btn btn-warning">
    ...
    @include('pages.admin.master.kecamatan.partials.modals')

    @push('scripts')
        {{ $dataTable->scripts() }}
        <script>
            const tableId = 'kecamatan-table';
